# 咨询链路恢复候选记录（2026-07-19）

> 状态：本地候选，未部署。本文记录代码和文件层面的诊断证据及发布后验收方法，不表示生产咨询链路已经恢复。
...
文件哈希和离线解码只能证明“候选文件未变”和“二维码编码了哪个 URL”，不能证明账号归属、账号仍可用、第三方应用能拉起或客服会收到消息。
...
- 生产 canary 必须返回 204，并写入隔离的 `index1 = 'seo_release_canary'`、`blob4 = 'seo_release_canary'` 与一次性 `blob5` 探针 ID。204 只证明处理器接受请求；受控生产发布随后必须通过 SQL API 精确回读同一个 ID，回读失败时不得报告 `deployed: true`。
