...
test("growth registry owns five indexable pages and seven evidence-gated pages", () => {
  assert.equal(GROWTH_PAGES.length, 12);
  assert.deepEqual(
  ...
  assert.equal(new Set(GROWTH_PAGES.map((page) => page.title)).size, 12);
  assert.equal(new Set(GROWTH_PAGES.map((page) => page.intent)).size, 12);
